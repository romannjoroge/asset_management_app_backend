...
export function getCurrentMarketValueOfAsset(barcode) {
        ...
        Asset._getAssetID(barcode).then(assetID => {
            ...
            getResultsFromDatabase(query, [assetID]).then(data => {
                if (data.length <= 0) {
                    ...
                    getResultsFromDatabase(query, [assetID]).then(acquisitionCostData => {
...
//# sourceMappingURL=currentMarketValue.js.map