...
export function storeAttachmentDetails(originalFileName: string, storageLocation: string, fileSize: number, userID: number, assetID: number, filetype: string): Promise<void> {
    return new Promise((res, rej) => {
        ...
        Asset._doesAssetIDExist(assetID).then(doesAssetExist => {
            ...
            User.checkIfUserIDExists(userID).then(doesUserExist => {
                ...
                pool.query(query, [assetID, userID, storageLocation, originalFileName, fileSize, filetype]).then((_: void) => {
                    return res();
                }).catch((err: any) => {
