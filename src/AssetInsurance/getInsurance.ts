...
import { AssetInsurance } from "./types.js";

export function getInsurances(barcode: string): Promise<AssetInsurance[]> {
    return new Promise((res, rej) => {
        Asset._getAssetID(barcode).then(assetID => {
            ...
            getResultsFromDatabase<AssetInsurance>(query, [assetID]).then(data => {
                return res(data);
            }).catch((err: MyError) => {
