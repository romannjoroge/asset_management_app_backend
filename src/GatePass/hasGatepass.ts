 ...
 */
export function doesAssetHaveGatepass(assetID: number, scannedTime: Date, fromLocation?: number, toLocation?: number): Promise<boolean> {
...
interface GetGatePassQueryResult {
    rows: GetGatePassQuery[];
    rowCount: number;
}

function checkIfGatepassValid(scannedTime: Date, assetID: number, fromLocation?: number, toLocation?: number): Promise<boolean> {
    return new Promise((res, rej) => {
        ...
        if (fromLocation != null && toLocation != null) {
            pool.query(db_gatepass.getGatepassWithLocationAndScannedTime, [assetID, beforeDate, afterDate, fromLocation]).then((result: GetGatePassQueryResult) => {
                if (result.rowCount <= 0) {
                    return res(false);
                }

                pool.query(db_gatepass.getGatepassWithLocationAndScannedTime, [assetID, beforeDate, afterDate, toLocation]).then((result2: GetGatePassQueryResult) => {
            ...
            pool.query(db_gatepass.getGatepassWithLocationAndScannedTime, [assetID, beforeDate, afterDate, toLocation]).then((result: GetGatePassQueryResult) => {
                if (result.rowCount <= 0) {
                    return res(false);
                }
