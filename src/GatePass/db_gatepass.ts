...
`
const getRequestedGatePasses = `SELECT g.id, u.name, (SELECT name AS fromlocation FROM Location WHERE id = g.fromlocation), (SELECT name AS tolocation FROM Location WHERE id = g.tolocation), g.date, a.barcode, g.reason, g.approved FROM Gatepass g JOIN gatepassasset ga ON ga.gatepassid = g.id LEFT JOIN User2 u ON u.id = g.userid JOIN Asset a ON a.assetid = ga.assetid WHERE g.id IN (SELECT gatepassid FROM AuthorizeGatepass WHERE userid = $1) AND g.approved = false;`;
